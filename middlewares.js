...
function verifyIdProducts(req, res, next) {
    ...
    .then((product) => {
        if(product !== 0) {
            ...
            res.status(404).json("No hay ningún producto registrado con ese id!");
        }
    });
};
...
function adminPermission(req, res, next) {
    ...
    .then((user) => {
        if(user[0].admin === 1) {
            ...
            res.status(401).json("No cuentas con los permisos necesarios para realizar esta operación!");
        }
    });
};
...
function validateInfoProduct(req, res, next) {
    ...
    if(!name || !price) {
        res.status(400).json("Ingresar todos los datos para crear un nuevo producto");
    } else {
        next();
    }
};
...
function validateInfoUser(req, res, next) {
    ...
    if(!userName || !password || !name || !lastname || !email || !telephone || !address || !admin) {
        res.status(400).json("Completar todos los datos para crear la cuenta");
    } else {
        next();
    }
};
...
function validateInfoOrder(req, res, next) {
    ...
    if(!id || !name || !price || !paymentMethod || !address) {
        res.status(400).json("Ingresar todos los datos para crear un nuevo pedido");
    } else {
        next();
    }
};
